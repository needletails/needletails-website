<template>
  <div class="page-layout-2">
    <div v-if="verified" class="page-layout-2-top">
      <h1 v-scrollanimation>Verified</h1>
    </div>
    <div v-else class="page-layout-2-top">
      <span><h1 v-scrollanimation>Error Verifying...</h1><h3><a href="mailto:inquiry@cartisim.io"> Contact Us</a></h3></span>
    </div>
...
export default {
  ...
  async created() {
    try {
      let v = await this.verifyEmail(this.$route.query.token);
      this.verified = true
      return v
    } catch (error) {
      this.loading = false;
  ...
  methods: {
    ...mapActions("Authentication", ["verifyEmail"]),
  },
};
</script>
<style scoped>
